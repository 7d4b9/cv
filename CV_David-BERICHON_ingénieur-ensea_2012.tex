...
\begin{section}{Expèrience professionnelle}
    ...
    \begin{entry}
        ...
        \Duration{4 mois}
        \Place{\bsc{Senior Backend Developper}}
        \Place{\href{https://jobteaser.com}{\bsc{Jobteaser}} \bsc{Senior Backend Developper}. Architecture microservice}
        \Locality{Paris}
        \Country{France}
        \Activity{- Backends Golang, Kafka et gRPC}
        \Activity{- CI groovy/Jenkins}
        \Activity{- BDD godog}
        \Activity{- Docker, K8s}
    \end{entry}
    ...
    \begin{entry}
        ...
        \Duration{15 mois}
        \Place{\bsc{Ingénieur développeur Back-End Golang}}
        \Place{\href{https://oyst.com}{\bsc{Oyst 1-Click}} \bsc{Ingénieur de développement Back-End Golang}. Architecture microservice sur AWS}
        \Locality{Paris}
        \Country{France}
        \Activity{- Référent Golang et développeur des workers en lien avec le paiement: design logiciel et impléméntation}
        \Activity{- Backends Golang, flux Kafka/Redis}
    ...
    \begin{entry}
        ...
        \Place{\href{https://www.ercom.fr}{\bsc{ERCOM-Cryptobox}} \bsc{Ingénieur développeur Golang et C++. Domaine de la sécurité :  solution de partage collaboratif avec chiffrement de bout en bout}}
        \Locality{Issy-Les-Moulineaux}
        \Country{France}
        \Activity{- Portage C++11 vers Golang du backend existant et d'un SDK multi-plateformes (iOS, Androïd, Windows, Web). En interaction avec une API REST : gestion de sessions utilisateur authentifiées, traitements cryptographiques, implémentation de protocoles propriétaires, synchronisation multi-clients, notifications}
        \Activity{- Génération du code SDK pour Java, Swift, C\# et JavaScript depuis Golang : gomobile, jsonrpc, gopherjs}
        \Activity{- Tests d'intégration avec RobotFramework + Python}
        \Activity{- Développement d'un CLI en Golang}
